...
func NewPGClient(conf *PGClientConfig) *PGClient {
	c := new(PGClient)
	c.Conf = conf
	pgSqlInfo := fmt.Sprintf("host=%s port=%d user=%s "+"password=%s dbname=%s sslmode=disable connect_timeout=5", conf.Host, conf.Port, conf.User, conf.Password, conf.DBName)
